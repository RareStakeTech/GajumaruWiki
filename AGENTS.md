...
## Output Layer (output/)
- Codex writes draft content here on `/repurpose` and `/content-plan` commands.
- Human edits and approves before publishing anywhere.
- NEVER publish `output/` content directly — always human-reviewed first.
- `output/` files use `status: draft | reviewed | published` in frontmatter.
- NEVER include unverified claims in `output/` — every claim must trace to a wiki page.
